...
export default function Example() {
  ...
  const LearnSection = () => {
    return (
      <>
      {learnMore ?  <p className="-mt-12 w-44 text-left text-gray-900 font-serif text-xs font-thin lg:mx-auto sm:text-sm md:text-base sm:w-60 md:w-72">
      ...
      :
      <>
      <p className="-mt-12 w-44 text-left text-gray-900 font-serif text-xs font-thin lg:mx-auto sm:text-sm md:text-base sm:w-60 md:w-72">
      Ayline’s music is far from expected. She brings an exciting vigor to the scene while keeping her roots true to the soulful blues that she grew up listening to and playing on her old upright piano; which she still sits at to write her music. With an emotionally piercing rock ’n roll voice that can effortlessly shift to a sensual ballad, nothing is predictable about her artistry.
      </p>
      <div className="mb-5 flex flex-row justify-between text-sm cursor-pointer text-gray-700 border-b-2 border-solid border-gray-600 w-32 ml-4" onClick={()=>setLearnMore(true)}>Learn More...<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
  ...
  return (
    <div id="music" className=" -mt-14 bg-white" style={{gridColumn:"span 7", gridRowStart:"third", gridRowEnd:"fourth"}}>
